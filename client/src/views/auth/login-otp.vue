...
import { resetForm, getDeviceInfo } from "@/helpers/helper";
...
const resendOtp = async () => {
  ...
  apiState.setSaving(true);
  try {
    const response = await authService.resendOTPApi({ email: form_data.email, process: 'login' });
    apiState.setSaving(false);
    apiState.setSuccess(true);
    apiState.setMessage(response.message);
  } catch (error: any) {
